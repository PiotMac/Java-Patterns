...
public abstract class CompositeShape extends Shape {
	...
	public List<Shape> getShapesByType(ShapeType type) {
		List<Shape> certainShapeType = new ArrayList<>();
		for (int i = 0; i < shapes.size(); i++) {
			if (shapes.get(i).getType() == type) {
				certainShapeType.add(shapes.get(i));
			}
		}
		return certainShapeType;
	}
	...
	public List<Shape> getLeafShapes() {
		...
		for (int i = 0; i < shapes.size(); i++) {
			if (!shapes.get(i).isComposite()) {
				leaves.add(shapes.get(i));
			}
		}
		return leaves;
	}
